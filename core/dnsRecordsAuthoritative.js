...
export async function forceRecords(target, failedRecords) {
    ...
    if (hasNS == true) {
        ...
        const remainingFailedRecords = await retryResolveRecords(target, nsRecords, failedRecords)
        //dig remaining records with ns 
        if (Object.keys(remainingFailedRecords).length > 0) {
            console.log("Remaining failed records after trying to get them with the NS records found:")
        ...
        if (hasSOA == true) {
            ...
            console.log("No NS record found, getting authoritative server from SOA record...")
 ...
 async function retryResolveRecords(target, nsRecords, failedRecords) {
    ...
    for (let z = 0; z < serverQuantity; z++) {
        ...
        await dnsPromises.resolve4(server,options).then(async(addressesV4)=>{
            ...
            await createDnsResolveFiles(target, `IPv4_addresses_${server}_`, addressesV4, true)
            const ipFromNsRecords = await getResolveDnsIpsFiles(target, true, server)
            
            for (let x = 0;x< ipFromNsRecords.length; x++) {
                resolver.setServers([ipFromNsRecords[x]["address"]])
                serverFailedRecords[server] = await getRecords(target, server, failedRecords, true) 
            }
            
        }).catch((err)=>{
            console.error("something went wrong getting the IPv4 addresses of the NS records" + err)
        })
    }
...
async function digNsRecord(target) {
    const { stdout, stderr } = await execAsync(`dig +nocmd ${target} NS +noall +answer`) 
    if (stderr) {
...
async function digRecords(target, remainingFailedRecords) {
    ...
    for (let i = 0; i < servers.length; i++) {
        ...
        for (let x = 0; x < records.length; x++) {
            ...
            const { stdout, stderr } = await execAsync(`dig +nocmd @${server} ${target} ${record} +noall +answer`) 
            if (stderr) {
        ...
        if (failedRecords[i] == "A") {
            ...
            await dnsPromises.resolve4(target,options).then((addressesV4)=>{
                ...
                if (isFromNsRecords == true) {
                    createDnsResolveFiles(target, `IPv4_addresses_${server}_`, addressesV4, isFromNsRecords)
                }
            }).catch((err)=>{
                if (err){
                    failedResolves.push("A")
                    console.error(err +" "+ server + " (Something went wrong resolving the IPv4's of the given DNS)");
                }
            })
            ...
            await dnsPromises.resolve6(target, options).then((addressesV6)=>{
                ...
                if (isFromNsRecords == true) {
                    createDnsResolveFiles(target, `IPv6_addresses_${server}_`, addressesV6, isFromNsRecords)
                }
            }).catch((err)=>{
                if (err){
                    failedResolves.push("AAAA")
                    console.error(err +" "+ server + " (Something went wrong resolving the IPv6's of the given DNS)");
                }
            })
            ...
            await dnsPromises.resolveCaa(target).then((recordsCaa)=>{
                ...
                if (isFromNsRecords == true) {
                    createDnsResolveFiles(target, `Caa_Records_${server}_`, recordsCaa, isFromNsRecords)
                }
            }).catch((err)=>{
                if (err){
                    failedResolves.push("CAA")
                    console.error(err +" "+ server + " (Something went wrong resolving the Caa records of the given DNS)");
                }
            })
            ...
            await dnsPromises.resolveCname(target).then((recordsCname)=>{
                ...
                if (isFromNsRecords == true) {
                    createDnsResolveFiles(target, `Cname_Records_${server}_`, recordsCname, isFromNsRecords)
                }
            }).catch((err)=>{
                if (err){
                    failedResolves.push("CNAME")
                    console.error(err +" "+ server + " (Something went wrong resolving the Cname records of the given DNS)");
                }
            })
            ...
            console.log("We got some Mx records for the Hostname!!")
                ...
                if (isFromNsRecords == true) {
                    createDnsResolveFiles(target, `Mx_Records_${server}_`, recordsMx, isFromNsRecords)
                }
            }).catch((err)=>{
                if (err) {
                    failedResolves.push("MX")
                    console.error(err +" "+ server + " (Something went wrong resolving the Mx records of the given DNS)");
                }
            })
            ...
            await dnsPromises.resolveNaptr(target).then((recordsNaptr)=>{
                ...
                if (isFromNsRecords == true) {
                    createDnsResolveFiles(target, `Naptr_Records_${server}_`, recordsNaptr, isFromNsRecords)
                }
            }).catch((err)=>{
                if (err){
                    failedResolves.push("NAPTR")
                    console.error(err +" "+ server + " (Something went wrong resolving the Naptr records of the given DNS)");
                }
            })
            ...
            await dnsPromises.resolveNs(target).then((recordsNs)=>{
                ...
                if (isFromNsRecords == true) {
                    createDnsResolveFiles(target, `Ns_Records_${server}_`, recordsNs, isFromNsRecords)
                }
            }).catch((err)=>{
                if (err){
                    failedResolves.push("NS")
                    console.error(err +" "+ server + " (Something went wrong resolving the Ns records of the given DNS)");
                }
            })
            ...
            await dnsPromises.resolvePtr(target).then((recordsPtr)=>{
                ...
                if (isFromNsRecords == true) {
                    createDnsResolveFiles(target, `Ptr_Records_${server}_`, recordsPtr, isFromNsRecords)
                }
            }).catch((err)=>{
                if (err) {
                    failedResolves.push("PTR")
                    console.error(err +" "+ server + " (Something went wrong resolving the Ptr records of the given DNS)");
                }
            })
            ...
            await dnsPromises.resolveSoa(target).then((recordsSoa)=>{
                ...
                if (isFromNsRecords == true) {
                    createDnsResolveFiles(target, `Soa_Records_${server}_`, recordsSoa, isFromNsRecords)
                }
            }).catch((err)=>{
                if (err){
                    failedResolves.push("SOA")
                    console.error(err +" "+ server + " (Something went wrong resolving the Soa records of the given DNS)");
                }
            })
            ...
            await dnsPromises.resolveSrv(target).then((recordsSrv)=>{
                ...
                if (isFromNsRecords == true) {
                    createDnsResolveFiles(target, `Srv_Records_${server}_`, recordsSrv, isFromNsRecords)
                }
            }).catch((err)=>{
                if (err){
                    failedResolves.push("SRV")
                    console.error(err +" "+ server + " (Something went wrong resolving the Srv records of the given DNS)");
                }
            })
            ...
            await dnsPromises.resolveTxt(target).then((recordsTxt)=>{
                ...
                if (isFromNsRecords == true) {
                    createDnsResolveFiles(target, `Txt_Records_${server}_`, recordsTxt, isFromNsRecords)
                } 
            }).catch((err)=>{
                if (err){
                    failedResolves.push("TXT")
                    console.error(err +" "+ server + " (Something went wrong resolving the Txt records of the given DNS)");
                }
            })
        }
    }
