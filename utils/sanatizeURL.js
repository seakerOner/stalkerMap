...
export function sanatizeURL(url) {
    ...
    if (scheme.substring(0,4) !== 'http' && scheme.substring(0.5) !== 'https') {
            throw new Error(`Invalid protocol (Valid protocols => http or https) (${url})`);
    }

    if (scheme.substring(0,7) !== 'http://' && scheme !== 'https://') {
            throw new Error(`Invalid scheme (Valid schemes => http:// or https://) (${url})`);
    }
...
function getPortAndtarget(protocol, fullURL, target, port) {
        ...
        return {"getFullURL":fullURL,"getProtocol":protocol,"getTargetType":targetType,"getTarget":target,"getPort":port}
    }